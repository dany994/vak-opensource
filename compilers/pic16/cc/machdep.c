...
int machif (node_t *n, int trueflag)
{
	...
	/* short var/!var */
	if (n->op == OP_NAME && tsize (n->type) == 2) {
		output (" xta %n; a|x %n+1;", n, n);
		if (trueflag) output (" nz?;");
		else          output (" z?;");
		return 1;
	}
	/* var != 0 */
	...
	/* short var/const <=/>/>=/< var/const */
	if ((n->op == OP_LE || n->op == OP_GT ||
	    n->op == OP_GE || n->op == OP_LT) &&
	    (n->left->op == OP_NAME || n->left->op == OP_CONST) &&
	    (n->right->op == OP_NAME || n->right->op == OP_CONST) &&
	    tsize (n->left->type) == 2 && tsize (n->right->type) == 2) {
		if (n->op == OP_GE || n->op == OP_LT) {
			node_t *x = n->left;
			n->left = n->right;
			n->right = x;
			n->op = (n->op == OP_GE) ? OP_LE : OP_GT;
		}
		if (n->op == OP_GT)
			trueflag = !trueflag;
		if (n->left->op == OP_NAME) output (" xta %n+1;", n->left);
		else			    output (" cta %h;", n->left);
		if (n->right->op == OP_NAME) output (" a-x %n+1;", n->right);
		else			     output (" c-a %h;", n->right);
		if (trueflag) output (" b?; goto 2f; nz?; goto 1f;");
		else	      output (" b?; goto 1f; nz?; goto 2f;");
		if (n->left->op == OP_NAME) output (" xta %n;", n->left);
		else			    output (" cta %l;", n->left);
		if (n->right->op == OP_NAME) output (" a-x %n;", n->right);
		else			     output (" c-a %l;", n->right);
		if (trueflag) output ("2: nb?;1:");
		else	      output ("2: b?;1:");
		return 1;
	}
	/* var/const >=/< expr */
...
void printunary (int op, int type, int argtype)
{
	...
	case 4:
		switch (op) {
		case OP_NOT:            /* logical negate op (!x) */
			output (" a|x A1; a|x A2; a|x A3; nz?; cta 1; a^c 1;");
			if (sz > 1)
				output (" xz A1;");
			break;
		case OP_COMPL:          /* logical complement op (~x) */
