 ...
 */
static void bulk_write (pickit2_adapter_t *a, unsigned char *output, int nbytes)
{
    int bytes_written;
 ...
 */
static void pickit2_flush_output (pickit2_adapter_t *a)
{
    int bytes_read, n;
    ...
    if (debug_level > 1) {
        int i;
        fprintf (stderr, "pickit2_flush_output received %d bytes:", a->bytes_to_read);
...
static void pickit2_send (pickit2_adapter_t *a,
...
static unsigned long long pickit2_fix_data (pickit2_adapter_t *a, unsigned long long word)
{
...
static unsigned long long pickit2_recv (pickit2_adapter_t *a)
{
    ...
    return pickit2_fix_data (a, word);
}

static void pickit2_reset (pickit2_adapter_t *a, int trst, int sysrst, int led)
{
    ...
    bulk_write (a, output, 3);
    if (debug_level)
        fprintf (stderr, "pickit2_reset (trst=%d, sysrst=%d) high_output=0x%2.2x, high_direction: 0x%2.2x\n",
            trst, sysrst, high_output, high_direction);
}

static void pickit2_speed (pickit2_adapter_t *a, int divisor)
{
    unsigned char output [3];
 ...
 * Запись регистра DP.
 */
static void pickit2_dp_write (adapter_t *adapter, int reg, unsigned value)
{
    pickit2_adapter_t *a = (pickit2_adapter_t*) adapter;

    pickit2_send (a, 1, 1, 4, JTAG_IR_DPACC, 0);
    pickit2_send (a, 0, 0, 32 + 3, (reg >> 1) |
 ...
 * Чтение регистра DP.
 */
static unsigned pickit2_dp_read (adapter_t *adapter, int reg)
{
    pickit2_adapter_t *a = (pickit2_adapter_t*) adapter;

    /* Читаем содержимое регистра DP. */
    pickit2_send (a, 1, 1, 4, JTAG_IR_DPACC, 0);
    pickit2_send (a, 0, 0, 32 + 3, (reg >> 1) | 1, 0);
    pickit2_send (a, 0, 0, 32 + 3, (DP_RDBUFF >> 1) | 1, 1);
    unsigned long long reply = pickit2_recv (a);
 ...
 */
static void pickit2_mem_ap_write (adapter_t *adapter, int reg, unsigned value)
{
    pickit2_adapter_t *a = (pickit2_adapter_t*) adapter;

    /* Пишем в регистр MEM-AP. */
    pickit2_send (a, 1, 1, 4, JTAG_IR_APACC, 0);
    pickit2_send (a, 0, 0, 32 + 3, (reg >> 1 & 6) |
        (unsigned long long) value << 3, 0);
    if (debug_level > 1) {
 ...
 * Старшие биты адреса должны быть предварительно занесены в регистр DP_SELECT.
 */
static unsigned pickit2_mem_ap_read (adapter_t *adapter, int reg)
{
    pickit2_adapter_t *a = (pickit2_adapter_t*) adapter;

    /* Читаем содержимое регистра MEM-AP. */
    pickit2_send (a, 1, 1, 4, JTAG_IR_APACC, 0);
    pickit2_send (a, 0, 0, 32 + 3, (reg >> 1 & 6) | 1, 0);
 ...
 */
static void pickit2_reset_cpu (adapter_t *adapter)
{
    pickit2_adapter_t *a = (pickit2_adapter_t*) adapter;
    ...
    pickit2_reset (a, 0, 1, 1);
    pickit2_reset (a, 0, 0, 1);
}
...
failed: usb_release_interface (a->usbdev, 0);
    ...
    if (usb_control_msg (a->usbdev,
        ...
        fprintf (stderr, "Can't set sync pickit2 mode\n");
        goto failed;
    }
    ...
    pickit2_speed (a, divisor);

    /* Disable TDI to TDO loopback. */
    unsigned char enable_loopback[] = "\x85";
    bulk_write (a, enable_loopback, 1);

    pickit2_reset (a, 1, 1, 1);
    pickit2_reset (a, 0, 0, 1);

    /* Reset the JTAG TAP controller. */
    pickit2_send (a, 6, 31, 0, 0, 0);         /* TMS 1-1-1-1-1-0 */

    /* Обязательные функции. */
    a->adapter.close = pickit2_close;
    a->adapter.get_idcode = pickit2_get_idcode;
    a->adapter.reset_cpu = pickit2_reset_cpu;
    a->adapter.dp_read = pickit2_dp_read;
    a->adapter.dp_write = pickit2_dp_write;
    a->adapter.mem_ap_read = pickit2_mem_ap_read;
    a->adapter.mem_ap_write = pickit2_mem_ap_write;
